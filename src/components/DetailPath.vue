  ...
  methods: {
    ...
    getDirection(nodes) {
      ...
      nodes.forEach((val, idx) => {
        ...
        if ((val.x - prevNode.x) * (val.y - nextNode.y) - (prevNode.y - val.y) * (nextNode.x - val.x) > 0) posNum++
          else negNum++
      })
    ...
    draw() {
      ...
      let vm = this
      // 添加箭头形状
      let arrowMarker=this.svg.append("defs").append("marker")
...
/*          console.log(vm.nodes.nodes(), vm.nodes, vm.graphData.nodes)
      ...
      function ticked() {
        ...
        arrowMarker.attr("refY", direction===DIRECTION.CLOCKWISE?-1.5:1.5)
      ...
      function getDirectArcPath(direction) {
        return function linkArc(d) {
          ...
          // let rotateDir=1
          return "M" + d.source.x + "," + d.source.y + "A" + dr + "," + dr + ` 0 0,${rotateDir} ` + d.target.x + "," + d.target.y;
        }
      }
